...
def calcBrownsCombinedP(snpSet, genotypes):
    ...
    runningSum = 0
    snpNames = [[i for i in snpSet.snps[j].keys()] for j in snpSet.snps.keys()]
    snpNames = snpNames[0]
    print snpNames
    snpPairs = ['{0}\t{1}'.format(i[0], i[1]) for i in itertools.combinations(snpNames, 2)]
    ...
    myRs = dict(zip(snpPairs, myRs))
    out = open('testingSNPRs.txt', 'w')
    for k,v in myRs.iteritems():
        out.write('{0}\t{1}\n'.format(k,v))
